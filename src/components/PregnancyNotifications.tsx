...
import { Baby, Heart, HeartPulse, Calendar, Gift, Plus } from 'lucide-react';
...
const PregnancyNotifications = () => {
  ...
  // بيانات الإشعارات النموذجية للحمل من بداية الحمل حتى الولادة
              ...
              .map(note => (
                ...
                >
                  <div className="flex items-center">
                    <div className="ml-2">
                      {!note.read && (
                        <span className="w-2 h-2 bg-kidmam-purple rounded-full absolute right-2 top-2" />
                      )}
                      <Heart className="h-3 w-3 text-pink-500" />
                    </div>
                    <div>
                      <div className="font-medium text-sm">{note.title}</div>
                      <div className="text-xs text-muted-foreground">اليوم {note.day}</div>
                    </div>
                  </div>
                </div>
              ))}
